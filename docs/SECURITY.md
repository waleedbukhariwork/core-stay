...
All profile reads, catalog reads and mutations authenticate at HTTP and enforce `IdentityApi.assertActiveVerified` again at the application boundary. The check rejects fabricated/expired actors, inactive/unverified accounts and revoked/expired or mismatched sessions. Ownership is derived only from the verified actor. Profile endpoints expose no cross-user selector and reject extra body fields, including ownership fields. The foreign key prevents orphan profiles; deletion of an account removes its profile.

The backend catalog and domain validation control selectable IDs, duplicates, required fields, time values and mutually exclusive primary learning approaches. Preference bodies are excluded by the existing request-log serializer; explicit responses contain no account IDs or persistence metadata. Network failures retain in-memory selections and secure session state. Profile responses cannot restore another account's state after logout/account change. Already authorized profile requests may complete concurrently with session revocation; subsequent requests perform a fresh live-session check.
